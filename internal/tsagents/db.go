...
func (ts *Server) InsertRecord(workflowID, workflowType string, data interface{}) error {
	now := time.Now().Format("2006-01-02 13:04:05")
...
func (ts *Server) ListRecords(pageIndex, pageSize int, workflowType, workflowID string) (*PageInfo, []*TSAgentExecutionRecords, error) {
	...
	query := ts.db.Model(&TSAgentExecutionRecords{}).Where("type = ? and workflow_id = ?", workflowType, workflowID)
